...
#define CP15_reg1_Ubit 0 //Non word-aligned addresses n'est pas support� dans notre impl�mentation donc valeur � 0
...
int arm_load_store(arm_core p, uint32_t inst) {
  ...
  uint8_t W = get_bit(inst, 21); // 21 si w = 1 r�crit la valeur dans celui de depart apr�s le decalage
...
int arm_load_store_halfword(arm_core p, uint32_t ins) {
    ...
    if (L==1) {
     ...
     arm_write_register( p, rd, data);
      ...
      // Ceci est n�cessaire pour les m�moires partag�es non impl�ment�es
